...
class SmsApiRepository {
  ...
  Future<SendSmsResponse> sendSms(String mobile, String message, {int? personId}) async {
    ...
    try {
      response = await http.post(uri, headers: const {
  ...
  SendSmsResponse _parseSendResponse(http.Response response) {
    ...
    if (entries is List && entries.isNotEmpty && entries.first is Map) {
      ...
      return SendSmsResponse(
        success: true,
        message: entryStatusText?.isNotEmpty == true ? entryStatusText! : (apiMessage?.isNotEmpty == true ? apiMessage! : 'پیامک با موفقیت به کاوه‌نگار ارسال شد.'),
        providerMessageId: providerMessageId,
      );
    }
    return SendSmsResponse(success: true, message: apiMessage?.isNotEmpty == true ? apiMessage! : 'پیامک با موفقیت ثبت شد.', providerMessageId: providerMessageId);
  }
  ...
  static int _toInt(dynamic value) => value is int ? value : int.tryParse(value?.toString() ?? '') ?? 0;
}
