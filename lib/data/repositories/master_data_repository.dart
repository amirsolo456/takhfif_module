...
class MasterDataRepository {
  ...
  Future<List<Person>> searchPersons(String query) async {
    ...
    return _extractList(decoded).map((e) => _personFromCustomerJson(Map<String, dynamic>.from(e))).toList(growable: false);
  }
  ...
  Future<Kala> createKala({required String code, required String name, double salePrice = 0, double purchasePrice = 0, String? barcode, int unitId = 1, int typeId = 1}) async {
    ...
    final data = decoded is Map<String, dynamic> && decoded['data'] is Map ? Map<String, dynamic>.from(decoded['data']) : Map<String, dynamic>.from(decoded as Map);
    return Kala.fromJson(data);
  }

  Future<Person> createPerson(Map<String, dynamic> data) async {
    final payload = <String, dynamic>{'personType': data['personType'], 'firstName': data['firstName'], 'lastName': data['lastName'], 'companyName': data['companyName'], 'mobile': data['mobile'], 'phone': data['phone'], 'address': data['address']};
    final uri = Uri.parse('$baseUrl/api/customers');
    final response = await http.post(uri, headers: const {'Accept': 'application/json', 'Content-Type': 'application/json'}, body: jsonEncode(payload));
    if (response.statusCode < 200 || response.statusCode >= 300) throw Exception(_apiErrorMessage(response, 'خطا در ثبت مشتری'));
    final decoded = jsonDecode(response.body) as Map<String, dynamic>;
    if (decoded['success'] == false) throw Exception((decoded['message'] ?? 'خطا در ثبت مشتری').toString());
    final payloadData = decoded['data'] is Map ? Map<String, dynamic>.from(decoded['data']) : decoded;
  ...
  String _apiErrorMessage(http.Response response, String fallback) {
    try {
      ...
      if (decoded is Map<String, dynamic>) {
        ...
        if (message != null && message.toString().trim().isNotEmpty) return '$message (HTTP ${response.statusCode})';
      }
    } catch (_) {}
    final body = response.body.trim();
    if (body.isNotEmpty && body.length < 500) return '$fallback (HTTP ${response.statusCode}): $body';
  ...
  Person _personFromCustomerJson(Map<String, dynamic> json) {
    ...
    return Person(id: _toInt(json['id']), personType: type, firstName: parts.isNotEmpty ? parts.first : '', lastName: parts.length > 1 ? parts.sublist(1).join(' ') : '', companyName: null, mobile: json['mobile']?.toString(), phone: json['phone']?.toString(), address: json['address']?.toString(), isActive: true);
  }

  int _toInt(dynamic v, {int fallback = 0}) => v is num ? v.toInt() : int.tryParse(v?.toString() ?? '') ?? fallback;
  List<dynamic> _extractList(dynamic decoded) {
    if (decoded is List) return decoded;
    if (decoded is Map<String, dynamic>) { final data = decoded['data']; if (data is List) return data; final result = decoded['result']; if (result is List) return result; }
    return const [];
  }
}
