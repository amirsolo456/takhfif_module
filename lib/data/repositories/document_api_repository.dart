...
class DocumentApiRepository {
  ...
  Future<List<DocumentModel>> getHistory({required int idSal, int sanadType = 12, int page = 1, int pageSize = 30}) async {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300 || !result.success) {
      throw DocumentApiException(code: result.code.isEmpty ? 'HTTP_${response.statusCode}' : result.code, message: result.message.isEmpty ? 'خطا در دریافت تاریخچه اسناد.' : result.message, errors: result.errors, warnings: result.warnings);
    }
    return result.data;
  }
  ...
  DocumentModel _parseDocumentResponse(http.Response response, {required String fallbackMessage}) {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300 || !result.success || result.data == null) {
      throw DocumentApiException(code: result.code.isEmpty ? 'HTTP_${response.statusCode}' : result.code, message: result.message.isEmpty ? fallbackMessage : result.message, errors: result.errors, warnings: result.warnings);
    }
    return result.data!;
  }
}
