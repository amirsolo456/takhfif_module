    ...
    return Scaffold(
      ...
      body: Directionality(
        ...
        child: _loading
            ...
            : _error != null
                ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Text(_error!), const SizedBox(height: 12), FilledButton(onPressed: _load, child: const Text('تلاش مجدد'))]))
                : _orders.isEmpty
                    ? const Center(child: Text('فاکتور در انتظار اقدامی وجود ندارد.'))
                    : RefreshIndicator(onRefresh: _load, child: ListView.builder(padding: const EdgeInsets.all(12), itemCount: _orders.length, itemBuilder: (_, i) => _orderCard(_orders[i]))),
      ),
    );
  }
...
class _PendingOrderSheet extends StatefulWidget {
  ...
  const _PendingOrderSheet({required this.order, required this.prices, required this.onSubmit});
  @override
