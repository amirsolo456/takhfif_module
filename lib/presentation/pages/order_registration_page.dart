...
class _OrderRegistrationPageState extends State<OrderRegistrationPage> {
  ...
  void _showPersonSearch(OrderRegistrationController controller) => showModalBottomSheet(
        context: context,
        isScrollControlled: true,
        builder: (_) => PersonSearchSheet(onSelected: (p) => setState(() => controller.selectedPerson = p)),
      );

  void _showKalaSearch(OrderRegistrationController controller) => showModalBottomSheet(
        context: context,
        isScrollControlled: true,
        builder: (_) => KalaSearchSheet(onSelected: (k) => controller.addToBasket(k)),
      );

  void _createNewPerson(OrderRegistrationController controller) async {
    final person = await Navigator.push<Person>(context, MaterialPageRoute(builder: (_) => const PersonFormPage()));
    if (person != null && mounted) setState(() => controller.selectedPerson = person);
  }
  ...
  void _submit(OrderRegistrationController controller) async {
    ...
    if (order != null && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        ...
        action: SnackBarAction(label: 'کپی خطا', textColor: Colors.white, onPressed: () => Clipboard.setData(ClipboardData(text: errorMsg))),
      ));
    }
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton.icon(onPressed: () { Navigator.pop(context); _showSmsDialog(order, controller); }, icon: const Icon(Icons.sms), label: const Text('ارسال پیامک به خریدار')),
        ],
      ),
    );
  }
...
abstract class _KeyboardSearchSheetState<T extends StatefulWidget> extends State<T> {
  ...
  @override
  void dispose() { searchFocusNode.dispose(); super.dispose(); }
  void showKeyboard() {
    if (!searchFocusNode.hasFocus) searchFocusNode.requestFocus();
    WidgetsBinding.instance.addPostFrameCallback((_) => SystemChannels.textInput.invokeMethod<void>('TextInput.show'));
  }
}
...
class _PersonSearchSheetState extends _KeyboardSearchSheetState<PersonSearchSheet> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) { showKeyboard(); _performSearch(''); });
  }
  @override
  void dispose() { _searchDebounce?.cancel(); _textController.dispose(); super.dispose(); }
  void _onQueryChanged(String value) {
    _currentQuery = value;
    _searchDebounce?.cancel();
    if (value.trim().isEmpty) { _performSearch(''); return; }
    _searchDebounce = Timer(const Duration(milliseconds: 300), () => _performSearch(value.trim()));
  }
  Future<void> _performSearch(String query) async {
    ...
    try {
      ...
      if (!mounted) return;
      if (_currentQuery.trim() == query) setState(() { _results = results; _searching = false; });
    } catch (e) {
      if (!mounted) return;
      if (_currentQuery.trim() == query) setState(() { _results = []; _searching = false; _error = e.toString(); });
    }
  }
  void _clearSearch() { _textController.clear(); _onQueryChanged(''); }
  Future<void> _createPersonFromSearch(OrderRegistrationController controller) async {
    final person = await Navigator.push<Person>(context, MaterialPageRoute(builder: (_) => PersonFormPage(initialSearch: _currentQuery)));
    if (person == null || !mounted) return;
    widget.onSelected(person); Navigator.pop(context);
  }
  @override
  Widget build(BuildContext context) {
    final controller = context.read<OrderRegistrationController>();
      ...
      child: Container(
        ...
        child: Column(children: [
          TextField(
            ...
            autofocus: true,
            decoration: InputDecoration(labelText: 'نام یا موبایل مشتری...', prefixIcon: const Icon(Icons.search), suffixIcon: hasQuery ? IconButton(icon: const Icon(Icons.clear), onPressed: _clearSearch) : null, border: const OutlineInputBorder()),
          ...
          const SizedBox(height: 8),
          Expanded(
            child: _searching ? const Center(child: CircularProgressIndicator()) : _results.isNotEmpty
                ? ListView.builder(itemCount: _results.length, itemBuilder: (context, i) {
                    final p = _results[i];
                    return ListTile(title: Text(p.fullName), subtitle: Text(p.mobile ?? ''), onTap: () { widget.onSelected(p); Navigator.pop(context); });
                  })
                : Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const SizedBox(height: 16),
                    ElevatedButton.icon(onPressed: () => _createPersonFromSearch(controller), icon: const Icon(Icons.person_add_alt_1), label: const Text('افزودن شخص جدید'), style: ElevatedButton.styleFrom(backgroundColor: Colors.green, foregroundColor: Colors.white)),
                  ])),
          ),
        ]),
      ),
...
class _KalaSearchSheetState extends _KeyboardSearchSheetState<KalaSearchSheet> {
  ...
  @override
  void initState() { super.initState(); WidgetsBinding.instance.addPostFrameCallback((_) { showKeyboard(); _performSearch(''); }); }
  @override
  void dispose() { _searchDebounce?.cancel(); _textController.dispose(); super.dispose(); }
  void _onQueryChanged(String value) {
    _currentQuery = value; _searchDebounce?.cancel();
    if (value.trim().isEmpty) { _performSearch(''); return; }
    _searchDebounce = Timer(const Duration(milliseconds: 300), () => _performSearch(value.trim()));
  }
  Future<void> _performSearch(String query) async {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(16),
        child: Column(children: [
          TextField(controller: _textController, focusNode: searchFocusNode, autofocus: true, decoration: InputDecoration(labelText: 'نام یا کد کالا...', prefixIcon: const Icon(Icons.search), suffixIcon: hasQuery ? IconButton(icon: const Icon(Icons.clear), onPressed: _clearSearch) : null, border: const OutlineInputBorder()), onChanged: _onQueryChanged),
          if (_searching) const LinearProgressIndicator(),
          if (_error != null) Padding(padding: const EdgeInsets.only(top: 12), child: Text(_error!, style: const TextStyle(color: Colors.red))),
          const SizedBox(height: 8),
          Expanded(
            child: _searching ? const Center(child: CircularProgressIndicator()) : _results.isNotEmpty
                ? ListView.builder(itemCount: _results.length, itemBuilder: (context, i) {
                    final k = _results[i];
                    return ListTile(title: Text(k.name), subtitle: Text('کد: ${k.code} | قیمت فروش: ${NumberFormat('#,###').format(k.salePrice ?? 0)} | خرید فعلی: ${NumberFormat('#,###').format(k.purchasePrice ?? 0)} ریال'), onTap: () { widget.onSelected(k); Navigator.pop(context); });
                  })
                : Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
