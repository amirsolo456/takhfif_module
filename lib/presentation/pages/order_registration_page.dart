...
class OrderRegistrationPage extends StatefulWidget {
  ...
  @override State<OrderRegistrationPage> createState() =>
      _OrderRegistrationPageState();
}
...
class _OrderRegistrationPageState extends State<OrderRegistrationPage> {
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        Row(children: [
          ...
          if (isDesktop) Container(width: 380,
        ...
        if (controller.isLoading) Container(color: Colors.black26,
  ...
  Widget _buildWorkflowTitle(String title) =>
      ...
      child: Row(children: [
        Container(width: 8, height: 22, decoration: BoxDecoration(color: Theme
            .of(context)
            .colorScheme
            .primary, borderRadius: BorderRadius.circular(99))),
  ...
  Widget _buildPersonSection(OrderRegistrationController controller) {
    ...
    return Card(elevation: 0,
        ...
        child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [
          Container(width: 48,
              height: 48,
              decoration: BoxDecoration(color: selected != null
                  ? theme.colorScheme.primaryContainer
                  : theme.colorScheme.surfaceContainerHighest,
                  borderRadius: BorderRadius.circular(14)),
              child: Icon(selected != null ? Icons.person_rounded : Icons
                  .person_add_alt_1_rounded,
                  color: selected != null ? theme.colorScheme.primary : theme
          ...
          Row(mainAxisSize: MainAxisSize.min,
              children: [
                IconButton.filledTonal(tooltip: 'مشتری جدید',
  ...
  Widget _buildKalaSearchSection(OrderRegistrationController controller) {
    ...
    return SizedBox(width: double.infinity,
        child: OutlinedButton.icon(onPressed: () => _showKalaSearch(controller),
          ...
          return Card(elevation: 0,
              ...
              child: Padding(
                ...
                Row(children: [
                  ...
                  Expanded(child: Text(item.kala.name, style: const TextStyle(
                      fontWeight: FontWeight.w900, fontSize: 15))),
                  IconButton(tooltip: 'حذف از سبد',
                      icon: const Icon(
                ...
                Row(children: [
                  ...
                  const Spacer(),
                  _buildSmallInput('قیمت فروش', item.unitPrice, (v) =>
                      controller.updateUnitPrice(index, v)),
                  const SizedBox(width: 8),
                  _buildSmallInput('قیمت خرید', item.purchasePrice, (v) =>
                      controller.updatePurchasePrice(index, v)),
                  const SizedBox(width: 8),
                  _buildSmallInput('تخفیف', item.discount, (v) =>
                      controller.updateDiscount(index, v))
                ]),
  ...
  Widget _buildDiscountToggle(OrderRegistrationController controller) =>
      Card(elevation: 0,
          shape: RoundedRectangleBorder(side: BorderSide(color: Theme
              .of(context)
              .colorScheme
              .outlineVariant), borderRadius: BorderRadius.circular(18)),
          child: Padding(padding: const EdgeInsets.all(8),
              child: Column(children: [
                SwitchListTile(title: const Text('استفاده از کد تخفیف',
                    style: TextStyle(fontWeight: FontWeight.w800)),
                    subtitle: const Text(
                        'در صورت داشتن کد تخفیف آن را اعمال کنید.'),
                    value: _useDiscountCode,
                    onChanged: (v) => setState(() => _useDiscountCode = v)),
                if (_useDiscountCode) Padding(
                    ...
                    child: Column(children: [
                      ...
                      if (controller.discountValidation != null) Padding(
        ...
        child: Padding(padding: const EdgeInsets.all(18),
            child: Column(children: [
              ...
              _priceRow('سود ناخالص تقریبی:', grossProfit,
                  color: grossProfit >= 0 ? Colors.green.shade700 : Colors.red
                      .shade700),
              const Divider(height: 24),
  ...
  Widget _priceRow(String label, double val,
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            ...
            Text('${NumberFormat('#,###').format(val)} ریال', style: TextStyle(
  ...
  Widget _buildSubmitButton(OrderRegistrationController controller) =>
      ...
      width: double.infinity,
      height: 54,
      child: FilledButton.icon(onPressed: () => _submit(controller),
          style: FilledButton.styleFrom(shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(8))),
          icon: const Icon(Icons.check_circle_rounded),
          label: const Text('ثبت و نهایی‌سازی سفارش', style: TextStyle(
  ...
  void _showPersonSearch(OrderRegistrationController controller) =>
      showModalBottomSheet(context: context,
          ...
          builder: (_) =>
              EnhancedPersonSearchSheet(onSelected: (p) =>
              setState(() => controller.selectedPerson = p)));

  void _showKalaSearch(OrderRegistrationController controller) =>
      showModalBottomSheet(context: context,
          isScrollControlled: true,
          builder: (_) =>
              EnhancedKalaSearchSheet(
  ...
  void _createNewDiscountCode(OrderRegistrationController controller) async {
    final result = await showModalBottomSheet(context: context,
        ...
        builder: (_) =>
        const Padding(
            padding: EdgeInsets.only(top: 20), child: DiscountCodeFormPage()));
    if (result is String) {
  ...
  void _submit(OrderRegistrationController controller) async {
    ...
    if (order != null && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          ...
          action: SnackBarAction(label: 'کپی خطا',
  ...
  void _showSuccessDialog(OrderModel order,
    ...
    showDialog(context: context,
        ...
        builder: (context) =>
            AlertDialog(title: const Icon(
            Icons.check_circle, color: Colors.green, size: 60),
            content: Column(mainAxisSize: MainAxisSize.min,
                children: [
                  const Text('سفارش با موفقیت ثبت شد', style: TextStyle(
                      fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  Text('شماره سفارش: ${order.id ?? order.orderNumber}'),
                  Text('مبلغ: ${NumberFormat('#,###').format(
                      order.totalAmount)} ریال')
                ]),
            actions: [
  ...
  void _showSmsDialog(OrderModel order,
      OrderRegistrationController controller) {
    showDialog(context: context,
        builder: (context) =>
            SmsDialog(
            ...
            discountCode: controller.discountValidation?.isValid == true
                ...
                : null)).then((_) => _resetPage(controller));
  }
...
class PersonSearchSheet extends StatefulWidget {
  ...
  @override State<PersonSearchSheet> createState() => _PersonSearchSheetState();
}
...
class KalaSearchSheet extends StatefulWidget {
  ...
  @override State<KalaSearchSheet> createState() => _KalaSearchSheetState();
}
