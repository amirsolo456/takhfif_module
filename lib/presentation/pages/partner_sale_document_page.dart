...
class _PartnerSaleDocumentPageState extends State<PartnerSaleDocumentPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final profit = _lines.fold<double>(
      0,
      (sum, line) => sum + line.quantity * (line.salePrice - line.partnerCost),
    );
    final theme = Theme.of(context);

    return Scaffold(
      appBar: AppBar(
        title: const Text('فروش از انبار همکار', style: TextStyle(fontWeight: FontWeight.w800)),
        centerTitle: true,
      ),
      body: Directionality(
            ...
            SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  _sectionTitle('۳', 'توضیحات', Icons.notes_rounded),
                  const SizedBox(height: 10),
                  Card(
                    elevation: 0,
                    child: Padding(
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      ...
                      label: const Text('ثبت نهایی فروش همکار', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
                    ),
                  ),
                  const SizedBox(height: 24),
                ],
  ...
  Widget _sectionTitle(String step, String title, IconData icon) {
    ...
    return Row(children: [
      Container(
        ...
        child: Center(child: Text(step, style: TextStyle(color: color, fontWeight: FontWeight.w900))),
      ),
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(_customer?.mobile ?? 'مشتری را انتخاب کنید.', style: TextStyle(fontSize: 12, color: theme.colorScheme.onSurfaceVariant)),
            ]),
          ),
  ...
  Widget _buildAddProductButton(ThemeData theme) {
    return SizedBox(
      ...
      child: OutlinedButton.icon(
        ...
        label: const Text('افزودن کالا', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
      ),
    );
  }
  ...
  Widget _emptyState(ThemeData theme) {
    return Container(
      ...
      child: Column(children: [
        ...
        Text('هنوز کالایی اضافه نشده است', style: TextStyle(fontWeight: FontWeight.w700, color: theme.colorScheme.onSurfaceVariant)),
      ]),
    );
  }
        ...
        child: Column(children: [
          ...
          Row(children: [
            Expanded(child: TextFormField(
              initialValue: line.quantity.toInt().toString(),
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: 'تعداد', border: OutlineInputBorder()),
              onChanged: (value) => line.quantity = double.tryParse(value.replaceAll(',', '')) ?? 0,
            )),
            const SizedBox(width: 10),
            Expanded(child: TextFormField(
              key: ValueKey('partner-sale-price-$index-${line.salePrice}'),
              ...
              onChanged: (value) => setState(() => line.salePrice = CurrencyFormatter.parse(value)),
            )),
          ]),
          const SizedBox(height: 10),
          TextFormField(
            ...
            inputFormatters: [CurrencyFormatter.inputFormatter],
            keyboardType: TextInputType.number,
            decoration: const InputDecoration(labelText: 'بهای همکار', suffixText: 'ریال', border: OutlineInputBorder()),
            onChanged: (value) => setState(() => line.partnerCost = CurrencyFormatter.parse(value)),
          ),
          const SizedBox(height: 10),
          Row(children: [
            Text('جمع:', style: TextStyle(color: theme.colorScheme.onSurfaceVariant)),
            const Spacer(),
            Text('${_money(total)} ریال', style: const TextStyle(fontWeight: FontWeight.w900)),
          ]),
        ]),
      ),
    );
  ...
  Widget _summaryCard(double total, double profit, ThemeData theme) {
    return Card(
      elevation: 0,
      ...
      child: Padding(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 8),
          _summaryRow('سود:', '${_money(profit)} ریال', bold: true, color: profit >= 0 ? Colors.green.shade700 : Colors.red.shade700),
          const SizedBox(height: 12),
          const Align(
            alignment: Alignment.centerRight,
            child: Text('این سند از موجودی انبار خودمان کم نمی‌کند و فقط برای ثبت فروش و سود استفاده می‌شود.', style: TextStyle(fontSize: 12)),
          ),
        ]),
      ),
    );
  ...
  Widget _summaryRow(String label, String value, {bool bold = false, Color? color}) {
    return Row(children: [
      ...
      const Spacer(),
      Flexible(child: Text(value, textAlign: TextAlign.left, style: TextStyle(fontWeight: bold ? FontWeight.w900 : FontWeight.w700, color: color))),
    ]);
  }
      ...
      setState(() {
        if (index >= 0) {
          ...
          _lines.add(_PartnerSaleLine(
            kala: picked!,
            salePrice: picked!.salePrice ?? 0,
            partnerCost: picked!.purchasePrice ?? 0,
          ));
        }
      });
    });
  ...
  Future<void> _submit() async {
    ...
    if (_lines.any((x) => x.quantity <= 0 || x.salePrice < 0 || x.partnerCost < 0)) {
      _message('تعداد، قیمت فروش و بهای همکار را بررسی کنید', true);
      return;
    }
    if (_lines.any((x) => x.salePrice == 0 || x.partnerCost == 0)) {
      _message('برای محاسبه صحیح سود، قیمت فروش و بهای همکار را کامل کنید', true);
      return;
    }
      ...
      await showDialog<void>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('فروش از انبار همکار با موفقیت ثبت شد', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18), textAlign: TextAlign.center),
            const SizedBox(height: 8),
            Text('شماره فاکتور: ${IranFormat.digits(doc.idFaktor)}\nمبلغ: ${_money(doc.totalAmount)} ریال', textAlign: TextAlign.center),
          ]),
  ...
  void _message(String text, bool error) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(text), backgroundColor: error ? Colors.red.shade700 : Colors.green.shade700, behavior: SnackBarBehavior.floating));
  }
}
...
class _PartnerSaleLine {
  ...
  double salePrice;
  double partnerCost;
  _PartnerSaleLine({required this.kala, this.quantity = 1, required this.salePrice, required this.partnerCost});
}
