...
class _ItemCard extends StatelessWidget {
  final DocumentItemModel item; const _ItemCard({required this.item});
  @override Widget build(BuildContext context) => Card(margin: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 6), child: ListTile(
    title: Text('کالا: ${IranFormat.digits(item.idKala)}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17.5)),
    subtitle: Padding(padding: const EdgeInsets.only(top: 6), child: Text('ردیف ${IranFormat.digits(item.id2)} • ${item.isIncoming ? 'ورود' : 'خروج'} • تعداد: ${IranFormat.number(item.quantity)}')),
    trailing: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [Text('خرید: ${CurrencyHelper.format(item.purchasePrice)}', style: const TextStyle(fontSize: 12.5)), const SizedBox(height: 2), Text('فروش: ${CurrencyHelper.format(item.unitPrice)}', style: const TextStyle(fontSize: 12.5)), const SizedBox(height: 2), Text('جمع: ${CurrencyHelper.format(item.totalAmount)}', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 14.5))]),
  )));
}
...
class _ErrorView extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.error_outline, size: 48), const SizedBox(height: 12), Text(message, textAlign: TextAlign.center), const SizedBox(height: 16), ElevatedButton.icon(onPressed: onRetry, icon: const Icon(Icons.refresh), label: const Text('تلاش مجدد'))])));
}
