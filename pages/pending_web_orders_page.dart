...
class _PendingWebOrdersPageState extends State<PendingWebOrdersPage> {
  ...
  Widget _summaryCard(int count, num total) => Container(
    ...
    child: Row(
      children: [
        Container(width: 46, height: 46, decoration: BoxDecoration(color: Theme.of(context).colorScheme.primary, borderRadius: BorderRadius.circular(14)), child: Icon(Icons.receipt_long_rounded, color: Theme.of(context).colorScheme.onPrimary)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('فاکتورهای وبسایت', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900)),
  ...
  Widget _buildBody(List<PendingWebOrder> visible) {
    ...
    if (_error != null && _orders.isEmpty) return _StateMessage(icon: Icons.cloud_off_rounded, title: 'دریافت فاکتورهای وبسایت ناموفق بود', message: _error!, actionText: 'تلاش مجدد', onAction: _load);
    if (visible.isEmpty) return _StateMessage(icon: _searchController.text.trim().isEmpty ? Icons.receipt_long_outlined : Icons.search_off_rounded, title: _searchController.text.trim().isEmpty ? 'فاکتور وبسایتی پیدا نشد' : 'نتیجه‌ای پیدا نشد', message: _searchController.text.trim().isEmpty ? 'سفارش‌های ثبت‌شده از سایت تا زمان تأیید در این بخش قرار می‌گیرند.' : 'عبارت جستجو را تغییر بده.');
      ...
      child: Padding(
        ...
        child: Column(children: [
          Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
            ...
            const _Pill(label: 'در انتظار بررسی', icon: Icons.schedule_rounded),
          ]),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: Colors.amber.shade900)),
        ],
      ),
    );
  }
